...
> **_NOTE_**: With RKE2, `HelmChartConfigs` manifests may fail if they are put in the `/kubernetes/manifests` section in the configuration directory. The
> best practice is to place any `HelmChartConfigs` in `/var/lib/rancher/rke2/server/manifests/` using [os-files](#operating-system-files).
