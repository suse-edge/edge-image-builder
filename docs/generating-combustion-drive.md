# Generating Combustion Drives

Two things are necessary to generate a combustion drive with EIB:
1. A definition file that describes the configuration to generate
1. A directory that contains any configuration files along with any other custom files that
   will be included in the generated combustion drive

> **_NOTE:_** By default on OSX, the Podman machine has only 2GB of RAM. You must manually increase the Podman machine
> resources to increase build speed. Example: ```podman machine set --cpus 8 --memory 8192```

Unlike for image building, the majority of the required fields for generating combustion drives are in the generate
command.
...
> **_NOTE:_** Since the new combustion drive feature does not require a base image, various operating system configurations
...
> **_NOTE:_** When generating combustion with EIB `elemental-register` and `elemental-system-agent` 
> must either be pre-installed as an RPM through a KIWI built imagem, or you must provide them  using [custom](#custom) 
> files, and then install during boot time using a custom script.
...
Default run, cached artifacts will reside under `_build/cache` on the host machine:
```shell
podman run --rm -it --privileged -v $(pwd):/eib \
$EIB_IMAGE  \
generate \
...
Default run with mounted cache directory, cached objects will reside under `$CACHE_DIR` on host machine.
...
Run with custom user mounted cache directory, cached artifacts will reside under `$CACHE_DIR` on host machine.
...
No cache run. Specified with `--cache=false`. Artifacts will not be cached, and previously cached artifacts will be ignored:
```shell
podman run --rm -it --privileged -v $(pwd):/eib \
$EIB_IMAGE  \
generate \
