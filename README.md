...
* `-v` - Used to mount a local directory (in this example, the value of $IMAGE_DIR) into the EIB container at `/eib`.
* `--definition-file` - Specifies which definition file to build. The path to this file will be relative to
...
* `--output` - Indicates the name of the file that EIB will build. This may only be a filename; the combustion drive will
