...
func loginToRegistry(url string, auth image.RegistryAuthentication, outputWriter io.Writer) error {
	args := []string{"login", url, "--username", auth.Username, "--password", auth.Password}
...
func (c *Combustion) populateRegistry(ctx *image.Context, images []string) error {
	...
	for _, registry := range ctx.ImageDefinition.EmbeddedArtifactRegistry.Registries {
		for _, c := range registry.Credentials {
			if err = loginToRegistry(registry.URL, c, logFile); err != nil {
